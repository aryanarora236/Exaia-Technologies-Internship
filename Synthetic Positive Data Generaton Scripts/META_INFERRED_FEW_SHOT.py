...
class PersonaFewShotGenerator:
    ...
    def load_embeddings(self) -> None:
        ...
        if 'comment_text' not in df.columns:
            df['comment_text'] = df.get(
                'text',
                pd.Series([f"Comment {i}" for i in range(len(df))])
            )
    ...
    def process_all_personas(self) -> pd.DataFrame:
        ...
        outfn = f"/home/yc663354/testing/GenerationScripts/FINALVERSION_5400_META_InferredFewShot.csv"
